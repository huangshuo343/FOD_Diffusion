...
class i2iLDM(DDPM):
    ...
    def consistency_loss(self, x, x_t_minus_1, alpha=50):
        ...
        #     print(X.max(), X.min())
        #     X = X.detach().cpu()
        #     X = make_grid(X, nrow=8, normalize=True, range=(-1, 1))  # nrow can be changed according to your need
        #     X = X.mul(255).clamp(0, 255).byte()
        #     X = X.permute(1, 2, 0).numpy().astype(np.uint8)
        #     print(X.shape)
        #     Image.fromarray(X).save(f"/ifs/loni/faculty/shi/spectrum/Student_2020/lzhong/MRI2CT_beta/trash/{name}.png")

        # # save x_t_minus_1 for visualization
        # save_vis(x_t_minus_1, "x_t_minus_1")
        ...
        for i in range(x.shape[1]):
            if i == mid:
                continue
            # # warping error between cond and cond_neighbor
            # M = 1. - torch.exp(-alpha * torch.nn.functional.mse_loss(x[:,mid,...], x[:,i,...], reduction='none'))
            # # # save M for visualization
            # # save_vis(M[:,None,...], f"M_{i}")
            # l1 = (x[:,i,...] - x_t_minus_1[:,0,...]).abs()
            # loss += (M * l1)
    ...
    def reconstruct_x_t_minus_1_with_noise_pred(self, x, t, model_out):
        assert x.shape == model_out.shape, f"x.shape: {x.shape}, model_out.shape: {model_out.shape}"
        # reconstruct x_t_minus_1 from model_output
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        if mid != 0:
            if self.parameterization == "x0":
                ...
                # reconstruct x_t_minus_1 from model_output
                # x_t_minus_1 = self.reconstruct_x_t_minus_1_with_noise_pred(x_noisy, t, model_output)
                # x_t_minus_1.clamp_(-1., 1.)
