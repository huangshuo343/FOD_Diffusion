...
class FeatExtractNet(nn.Module):
    def __init__(
        ...
        self.conv_in = [1, 5, 9, 13, 17]
        self.conv_in_accum = [0, 1, 6, 15, 28, 45]
        assert self.in_channels == sum(self.conv_in), f"self.in_channels = {self.in_channels} should be sum(self.conv_in) = {sum(self.conv_in)}"
        assert self.out_channels == len(self.conv_in), f"self.out_channels = {self.out_channels} should be len(self.conv_in) = {len(self.conv_in)}"
        
        self.model_channels = model_channels

        self.input_blocks = nn.ModuleList(
            [
                conv_nd(dims, in_c, 1, 1, padding=0, bias=False) for in_c in self.conv_in #conv_nd(dims, in_c, 1, 3, padding=1) for in_c in self.conv_in
            ]
        )

        self.act1 = nn.SiLU()

        self.input_blocks_2 = nn.ModuleList(
            [
                conv_nd(dims, 5, 1, 3, padding=1) #conv_nd(dims, 5, 1, 1, padding=0)
            ]
        )

        self.act2 = nn.SiLU()

        self.volume_embeder = nn.Sequential(
            linear(self.model_channels*4, in_channels),
        )

    def forward(self, x, timesteps=None, volume_enconding=None, **kwargs):
        # t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        # emb = self.time_embed(t_emb)
        assert type(volume_enconding) is not type(None)

        volume_emb = torch.concat([timestep_embedding(volume_enconding[:, 0], self.model_channels, repeat_only=False),
        timestep_embedding(volume_enconding[:, 1], self.model_channels, repeat_only=False),
        timestep_embedding(volume_enconding[:, 2], self.model_channels, repeat_only=False),
        timestep_embedding(volume_enconding[:, 3], self.model_channels, repeat_only=False)], axis=1)

        v_emb = self.volume_embeder(volume_emb)
        x = x + v_emb[..., None, None, None]
        # for module in self.input_blocks:
        #     x = module(x)
        h = []
        for i, module in enumerate(self.input_blocks):
            h.append(module(x[:, self.conv_in_accum[i]:self.conv_in_accum[i+1], ...]))
            h[-1] = self.act1(h[-1])
        x = torch.cat(h, dim=1)
        #x = SiLU(x)
        for i, module in enumerate(self.input_blocks_2):
            x = module(x)
        x = self.act2(x)
        #x = SiLU(x)
        return x, volume_emb

class FeatExpandNet(nn.Module):
    def __init__(
            self,
            dims=3,
            in_channels=1,
...
class v2vLDM(DDPM):
    ...
    def __init__(self, feat_extract_stage_config, feat_expand_stage_config, control_key, mask_key="", *args, **kwargs):
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        if cond['c_concat'] is None:
            ...
            # control_noisy = self.q_sample(x_start=torch.cat(cond['c_concat'], 1), t=t, noise=None)
            # concat in channel dim

            # target_vol_idx = 34 # TODO
            # target_vol = cond['c_concat'][0][:, target_vol_idx:target_vol_idx+1, ...]
            ...
            control_input = torch.cat([one_ch_feat, one_ch_feat], 1)
            #_, controls = diffusion_model(x=control_input, timesteps=t, volume_enconding=volume_emb, context=cond_txt)
            _, controls = diffusion_model(x=control_input, timesteps=torch.zeros_like(t), volume_enconding=volume_emb, context=cond_txt) #t
            # controls = [c.detach() for c in controls]

            # attn_input = self.feat_expand_net(cond['attn_input'])
