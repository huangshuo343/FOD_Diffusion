...
class v2vLDM(DDPM):
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        #print('The shape of model_output is: ', model_output.shape, '.')
    ...
    def upper_branch(self, model, t, cond, volume_emb):
        vols_5 = []
        conv_in = [1, 5, 9, 13, 17]
        ...
        for i in range(vols_5.shape[1]):
            ...
            _, controls = model(x=tmp_input, timesteps=torch.zeros_like(t), volume_enconding=volume_emb, context=None) # TODO: check timesteps = 0 or t
    ...
    def apply_model(self, x_noisy, t, cond, test_cache=None, *args, **kwargs):
        ...
        if cond['c_concat'] is None:
            ...
            if test_cache is None:
                controls = self.upper_branch(diffusion_model, t, cond, volume_emb)
            else:
                controls = test_cache
    ...
    @torch.no_grad()
    def p_mean_variance(self, x, t, cond, clip_denoised: bool, test_cache=None):
        """
        Get the distribution p(x_{t-1} | x_t).
        """
        model_out = self.apply_model(x, t, cond=cond, test_cache=test_cache)
    ...
    @torch.no_grad()
    def p_sample(self, X, t, cond, clip_denoised=True, test_cache=None):
        """
        Sample from the model p(x_{t-1} | x_t).
        param x: x_t
        """
        b, c, x, y, z = X.shape
        model_mean, _, model_log_variance = self.p_mean_variance(x=X, t=t, cond=cond, clip_denoised=clip_denoised, test_cache=test_cache)
