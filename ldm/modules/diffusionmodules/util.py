...
def is_zero_tensor(tensor):
    return tensor is not None and torch.allclose(tensor, torch.zeros_like(tensor))
...
class CheckpointFunction(torch.autograd.Function):
    ...
    """This piece of * does not support None type input, which may happen while unconditioning"""
    @staticmethod
    def backward(ctx, *output_grads):
        # making f*king pytorch backward happy
        ctx.input_tensors = [x.detach().requires_grad_(True) if x is not None and not is_zero_tensor(
            x) else torch.tensor(0., requires_grad=True) for x in ctx.input_tensors]
